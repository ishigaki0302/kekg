...
### 1. ナレッジグラフ生成（no_alias版）

```bash
python src/kg/generator_no_alias.py \
    --num-entities 1200 \
    --num-relations 250 \
...
./run/run_training.sh
# または設定ファイルを指定
./run/run_training.sh --config configs/train_gpt_small_no_alias.yaml
```
...
# 結果の可視化
./run/run_sequential_analysis.sh --output-dir outputs/sequential_no_alias
```
...
# 比較プロットのみ再生成
./run/compare_results.sh --base-dir outputs/sequential_comparison_no_alias
```
...
./run/run_hop_analysis.sh \
    --base-dir outputs/degree_exclusive_multi_trial_no_alias_2 \
    --num-trials 10
```
...
│
├── configs/                          # 設定ファイル（YAML）
│   ├── train_gpt_small_no_alias.yaml # モデル学習設定（最新）
│   ├── train_gpt_small.yaml          # モデル学習設定（旧版）
│   └── kg_ba.yaml                    # BA グラフ KG 設定
│
├── src/
│   ├── kg/                           # KG 生成
│   │   ├── generator_no_alias.py     # BA KG 生成（最新）
│   │   └── generator.py              # BA/ER KG 生成（旧版）
│   ├── modeling/                     # モデル定義・学習
│   │   ├── gpt_mini.py               # GPTMini
...
│
├── data/
│   ├── kg/ba_no_alias/               # no_alias 版 KG（最新）
│   └── kg/ba/                        # alias 版 KG（旧版）
│
├── tests/                            # テスト
