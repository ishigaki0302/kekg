...
class KnowledgeGraph:
    """Knowledge graph built from a list of triples (no-alias version).

    In the no-alias design every entity/relation is its own unique token, so
    ``entity_aliases[e]`` and ``relation_aliases[r]`` simply return ``[e]``
    and ``[r]`` respectively.  This keeps downstream code that was written for
    the alias-aware API working without modification.
    """
    ...
    def entity_aliases(self) -> Dict[str, List[str]]:
        return {e: [e] for e in self.entities}

    @property
    def relation_aliases(self) -> Dict[str, List[str]]:
        return {r: [r] for r in self.relations}

    @classmethod
    def from_triples(cls, triples: List[Triple]) -> "KnowledgeGraph":
        """Build a KnowledgeGraph by inferring entities/relations from triples."""
        entities = sorted({t.s for t in triples} | {t.o for t in triples})
        relations = sorted({t.r for t in triples})
        return cls(entities=entities, relations=relations, triples=triples)
...
def main() -> None:
    ...
    ap.add_argument("--out-txt", type=str, default="data/kg/ba/corpus.train.txt")
    ap.add_argument("--out-jsonl", type=str, default="data/kg/ba/graph.jsonl")
