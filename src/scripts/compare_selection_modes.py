...
def plot_comparison(
    ...
    Args:
        base_dir: Base directory containing mode subdirectories
        output_path: Output path for the comparison figure
        xscale: "linear" or "log" (log uses step+1 to avoid log(0))
    """
    base = Path(base_dir)

    def _xvals(step_series: pd.Series) -> pd.Series:
        """Return x values for plotting steps.

        For log-scale plots, use step+1 to avoid log(0).
        """
        if xscale == "log":
            return step_series.astype(float) + 1.0
        return step_series.astype(float)

    def _apply_xaxis(ax: plt.Axes) -> None:
...
def main():
    ...
    # Generate comparison plots (linear + log-x)
    plot_comparison(args.base_dir, args.output, xscale="linear")

    out_p = Path(args.output)
    log_out = str(out_p.with_name(out_p.stem + "_xlog" + out_p.suffix))
    plot_comparison(args.base_dir, log_out, xscale="log")
